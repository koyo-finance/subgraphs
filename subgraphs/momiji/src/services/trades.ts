...
import { createOrUpdatePair } from './pairs';
import { getOrCreateSettlement } from './settlements';
import { createTokenTradingEvent } from './tokens';
import { addVolumesAndFees } from './totals';
import { getOrCreateSolver, getOrCreateTrader } from './users';
...
export function getOrCreateTrade(event: Trade, buyToken: Token, sellToken: Token): void {
	...
	getOrCreateSettlement(txHash, timestamp, solver, txGasPrice, feeAmountUsd);
	...
	createTokenTradingEvent(timestamp, buyTokenId, tradeId, buyAmount, buyAmountEth, buyAmountUsd, buyTokenPriceUsd);
	createTokenTradingEvent(timestamp, sellTokenId, tradeId, sellAmount, sellAmountEth, sellAmountUsd, sellTokenPriceUsd);
	...
	getOrCreateTrader(owner, timestamp, ownerAddress, ethAmountForVolumes, usdAmountForVolumes);
	getOrCreateSolver(solver, ethAmountForVolumes, usdAmountForVolumes);

	addVolumesAndFees(ethAmountForVolumes, usdAmountForVolumes, feeAmountEth, feeAmountUsd, timestamp);
